...
export const ListingCreateBookingModal = ({
  ...
  const [createBooking, { loading }] = useMutation<
    CreateBookingData, 
    CreateBookingVariables>(CREATE_BOOKING, {
      onCompleted: () => {
        clearBookingData();
        displaySuccessNotification(
      ...
      onError: () => {
        displayErrorMessage("Sorry! We are not able to successfully book the listing. Plase try again later!")
      }
    })
  ...
  const handleCreateBooking = async () => {
    if(!stripe){
      return displayErrorMessage('Sorry! We we are not able to connect with Stripe.');
    }
